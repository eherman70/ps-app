...
function SaleNumberManagement() {
  ...
  const [form, setForm] = useState({ marketCenterId: '', saleNumber: '', status: 'Active' });
  const [editing, setEditing] = useState(null);

  const handleSubmit = async () => {
    if (!form.marketCenterId || !form.saleNumber) {
  ...
  const resetForm = () => {
    setForm({ marketCenterId: '', saleNumber: '', status: 'Active' });
    setEditing(null);
    setShowForm(false);
  };

  const handleEdit = (item) => {
    setForm({ marketCenterId: item.marketCenterId, saleNumber: item.saleNumber, status: item.status });
    setEditing(item);
    setShowForm(true);
  };
